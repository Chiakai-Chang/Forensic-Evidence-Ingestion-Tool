...
本工具完全移除了舊版 VBScript/VB6 的 `Microsoft.VisualBasic` 舊版元件，改用現代 **Win32 API (`MessageBoxTimeout`)** 進行對話框調用，外觀與 Windows 10/11 系統風格完全一致，且能**安全繞過端點資安防護軟體（EDR）的惡意 VBScript 攔截規則**。視窗提供 10 秒自動倒數，時間到若無手動取消即自動同意開始進行目的地二次雜湊驗算。
